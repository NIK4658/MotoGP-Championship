...
public class AssegnaCostruttore extends JPanel{

    public AssegnaCostruttore(final Dimension dim, final DBConnection con) {
        String[] fields = {"CodiceTeam", "CodiceCostruttore"};
        ...
        //title
        JLabel title = new JLabel("Assegna COSTRUTTORE");
        title.setFont(new Font("Arial", Font.BOLD, dim.width / 15));
        title.setForeground(Color.BLACK);
        northPanel.add(title);
        //Main label wih text
        JLabel text = new JLabel("Perfavore seleziona i codici per assegnare un COSTRUTTORE ad un TEAM", JLabel.CENTER);
        ...
        for(String string : fields){
            ...
            if(i==1){
                textField = new JComboBox<String>(con.getCostruttori());
            } else {
        ...
        //button
        JButton register = new JButton("Effettua assegnazione");
        ...
        register.addActionListener((e) -> {
            ...
            String costruttString = ((JComboBox<String>)components[1]).getSelectedItem().toString();
            if(costruttString.length()>0 && costruttString.length()<=20 && TeamString.length()>0 && TeamString.length()<=20){
                if(con.AssegnaCostruttore(TeamString, costruttString)){
