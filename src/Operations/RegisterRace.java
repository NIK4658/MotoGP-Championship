...
public class RegisterRace extends JPanel{

    public RegisterRace(final Dimension dim, final DBConnection con) {
        String[] fields = {"Anno", "CodiceCampionato", "CodiceAutodromo", "Data", "Numero Giri"};
        ...
        //title
        JLabel title = new JLabel("Registrazione GARA");
        title.setFont(new Font("Arial", Font.BOLD, dim.width / 15));
        title.setForeground(Color.BLACK);
        northPanel.add(title);
        //Main label wih text
        JLabel text = new JLabel("Perfavore riempi i campi sottostanti per creare una nuova GARA", JLabel.CENTER);
        ...
        register.addActionListener((e) -> {
            String YearString = ((JTextField)components[0]).getText();
            ...
            if(champString.length()>0 && YearString.length()==4 && AutodromeString.length()>0 && NumberLapsString.length()>0 && NumberLapsString.length()<=16 && champString.length()<=20 && AutodromeString.length()<=20 && con.validDate(dateString)){
                ...
                error.setText("Registrazione fallita SUBITO!");
            }
        });
